...
let api_url = "httpps://api.github.com/users/shikhilrane";
...
// 1. async-await working as same as .then() and .catch()
...
async function consumePromiseOne() {
   try {                                  // Begin a try block to handle successful execution of the asynchronous operation
      let response = await promiseOne;    // Wait for the promise named 'promiseOne' to resolve and assign its result to the variable 'response'. If the promise is rejected, control will immediately jump to the catch block
      console.log(response);              // Log the resolved value of 'promiseOne' to the console
   } catch (error) {                      // Begin a catch block to handle any error that occurs while waiting for the promise
      console.error(error);               // Log the error to the console if the promise was rejected or an error occurred in the try block
   }
}
consumePromiseOne();